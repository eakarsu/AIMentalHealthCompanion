...
// Route: /api/gap-wearable
// Feature: Wearable
// Description: Wearable device sync (Apple Health, Fitbit, Whoop) beyond generic integrations
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Wearable. Wearable device sync (Apple Health, Fitbit, Whoop) beyond generic integrations' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'wearable', title: 'Wearable', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'wearable', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['wearable']);
      return res.json({ slug: 'wearable', items: r.rows || [] });
    }
    return res.json({ slug: 'wearable', items: [] });
  } catch (e) {
    return res.json({ slug: 'wearable', items: [], error: (e && e.message) || 'Server error' });
  }
});
