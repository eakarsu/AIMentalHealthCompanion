// Auto-generated feature route: companion-agent-24-7
// Domain: 24/7 mental health companion — Capability: running crisis de-escalation and therapist escalation
// Mounted under: /api/companion-agent-24-7
...
// POST /api/companion-agent-24-7/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a 24/7 mental health companion. Your task is running crisis de-escalation and therapist escalation. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/companion-agent-24-7/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/companion-agent-24-7/:id — fetch one
...
// DELETE /api/companion-agent-24-7/:id — discard a run
