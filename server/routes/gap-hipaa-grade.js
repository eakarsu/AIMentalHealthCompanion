...
// Route: /api/gap-hipaa-grade
// Feature: Hipaa Grade
// Description: HIPAA-grade compliance attestation/audit
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Hipaa Grade. HIPAA-grade compliance attestation/audit' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'hipaa-grade', title: 'Hipaa Grade', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'hipaa-grade', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['hipaa-grade']);
      return res.json({ slug: 'hipaa-grade', items: r.rows || [] });
    }
    return res.json({ slug: 'hipaa-grade', items: [] });
  } catch (e) {
    return res.json({ slug: 'hipaa-grade', items: [], error: (e && e.message) || 'Server error' });
  }
});
