// Auto-generated feature route: mood-anomaly-stream
// Domain: mood anomaly streamer — Capability: detecting anomalies across journal, mood, and chat signals
...
// POST /api/mood-anomaly-stream/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a mood anomaly streamer. Your task is detecting anomalies across journal, mood, and chat signals. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/mood-anomaly-stream/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/mood-anomaly-stream/:id — fetch one
...
// DELETE /api/mood-anomaly-stream/:id — discard a run
