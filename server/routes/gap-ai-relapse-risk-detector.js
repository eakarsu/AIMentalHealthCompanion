...
// Route: /api/gap-ai-relapse-risk-detector
// Feature: Ai Relapse Risk Detector
// Description: /ai/relapse-risk-detector
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Relapse Risk Detector. /ai/relapse-risk-detector' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Relapse Risk Detector\nDescription: /ai/relapse-risk-detector\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-relapse-risk-detector', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-relapse-risk-detector', title: 'Ai Relapse Risk Detector', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-relapse-risk-detector', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-relapse-risk-detector']);
      return res.json({ slug: 'ai-relapse-risk-detector', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-relapse-risk-detector', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-relapse-risk-detector', items: [], error: (e && e.message) || 'Server error' });
  }
});
