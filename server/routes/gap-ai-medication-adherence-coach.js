...
// Route: /api/gap-ai-medication-adherence-coach
// Feature: Ai Medication Adherence Coach
// Description: /ai/medication-adherence-coach
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Medication Adherence Coach. /ai/medication-adherence-coach' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Medication Adherence Coach\nDescription: /ai/medication-adherence-coach\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-medication-adherence-coach', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-medication-adherence-coach', title: 'Ai Medication Adherence Coach', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-medication-adherence-coach', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-medication-adherence-coach']);
      return res.json({ slug: 'ai-medication-adherence-coach', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-medication-adherence-coach', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-medication-adherence-coach', items: [], error: (e && e.message) || 'Server error' });
  }
});
