// Auto-generated feature route: cbt-dbt-agent
// Domain: CBT/DBT delivery agent — Capability: personalizing homework and tracking completion
// Mounted under: /api/cbt-dbt-agent
...
// POST /api/cbt-dbt-agent/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a CBT/DBT delivery agent. Your task is personalizing homework and tracking completion. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/cbt-dbt-agent/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/cbt-dbt-agent/:id — fetch one
...
// DELETE /api/cbt-dbt-agent/:id — discard a run
