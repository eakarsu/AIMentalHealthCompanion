// Auto-generated feature route: voice-therapy-access
// Domain: voice-based therapy access agent — Capability: supporting voice journaling, guided meditations, counselor calls
// Mounted under: /api/voice-therapy-access
...
// POST /api/voice-therapy-access/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a voice-based therapy access agent. Your task is supporting voice journaling, guided meditations, counselor calls. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/voice-therapy-access/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/voice-therapy-access/:id — fetch one
...
// DELETE /api/voice-therapy-access/:id — discard a run
