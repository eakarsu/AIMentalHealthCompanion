...
// Route: /api/gap-ai-cbt-exercise-generator
// Feature: Ai Cbt Exercise Generator
// Description: (personalized CBT homework)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Cbt Exercise Generator. (personalized CBT homework)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Cbt Exercise Generator\nDescription: (personalized CBT homework)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-cbt-exercise-generator', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-cbt-exercise-generator', title: 'Ai Cbt Exercise Generator', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-cbt-exercise-generator', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-cbt-exercise-generator']);
      return res.json({ slug: 'ai-cbt-exercise-generator', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-cbt-exercise-generator', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-cbt-exercise-generator', items: [], error: (e && e.message) || 'Server error' });
  }
});
