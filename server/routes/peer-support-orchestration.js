// Auto-generated feature route: peer-support-orchestration
// Domain: peer support orchestrator — Capability: facilitating peer groups agentically
// Mounted under: /api/peer-support-orchestration
...
// POST /api/peer-support-orchestration/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a peer support orchestrator. Your task is facilitating peer groups agentically. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
    ...
    const record = {
      ...
      slug: 'peer-support-orchestration',
      input: payload,
      output: parsed,
...
// GET /api/peer-support-orchestration/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/peer-support-orchestration/:id — fetch one
...
// DELETE /api/peer-support-orchestration/:id — discard a run
